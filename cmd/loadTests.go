...
var (
	...
	AveragePipelineRunSucceededTimePerUser []time.Duration
	AveragePipelineRunFailedTimePerUser    []time.Duration
	SuccessfulUserCreations                []int64
	SuccessfulResourceCreations            []int64
	SuccessfulPipelineRuns                 []int64
...
func setup(cmd *cobra.Command, args []string) {
	...
	AveragePipelineRunSucceededTimePerUser = make([]time.Duration, threadCount)
	AveragePipelineRunFailedTimePerUser = make([]time.Duration, threadCount)
	SuccessfulUserCreations = make([]int64, threadCount)
	SuccessfulResourceCreations = make([]int64, threadCount)
	SuccessfulPipelineRuns = make([]int64, threadCount)
	...
	userCreationSuccessCount := sumFromArray(SuccessfulUserCreations)
	if userCreationSuccessCount > 0 {
		averageTimeToSpinUpUsers = sumDurationFromArray(AverageUserCreationTime).Seconds() / float64(userCreationSuccessCount)
	}
	...
	resourceCreationSuccessCount := sumFromArray(SuccessfulResourceCreations)
	if resourceCreationSuccessCount > 0 {
		averageTimeToCreateResources = sumDurationFromArray(AverageResourceCreationTimePerUser).Seconds() / float64(resourceCreationSuccessCount)
	}
	...
	pipelineRunSuccessCount := sumFromArray(SuccessfulPipelineRuns)
	if pipelineRunSuccessCount > 0 {
		averageTimeToRunPipelineSucceeded = sumDurationFromArray(AveragePipelineRunSucceededTimePerUser).Seconds() / float64(pipelineRunSuccessCount)
	}
	...
	if pipelineRunFailureCount > 0 {
		averageTimeToRunPipelineFailed = sumDurationFromArray(AveragePipelineRunFailedTimePerUser).Seconds() / float64(pipelineRunFailureCount)
	}
	...
	go func() {
		...
		for userIndex := 1; userIndex <= numberOfUsers; userIndex++ {
			...
			if err != nil {
				...
				FailedUserCreations[threadIndex] += 1
			...
			SuccessfulUserCreations[threadIndex] += 1
			increaseBar(usersBar, usersBarMutex)
		}
		close(chUsers)
	}()
	...
	go func() {
		...
		for username := range chUsers {
			...
			if errors != nil {
				...
				FailedResourceCreations[threadIndex] += 1
				increaseBar(resourcesBar, resourcesBarMutex)
				continue
			}
			...
			SuccessfulResourceCreations[threadIndex] += 1
		...
		go func() {
			...
			for username := range chPipelines {
				...
				if err != nil {
					...
					FailedPipelineRuns[threadIndex] += 1
					increaseBar(pipelinesBar, pipelinesBarMutex)
					continue
				}
				...
				err = k8swait.Poll(pipelineRunRetryInterval, pipelineRunTimeout, func() (done bool, err error) {
					...
					if pipelineRun.IsDone() {
						...
						if succeededCondition.IsFalse() {
							...
							AveragePipelineRunFailedTimePerUser[threadIndex] += dur
							...
							dur := pipelineRun.Status.CompletionTime.Sub(pipelineRun.CreationTimestamp.Time)
							AveragePipelineRunSucceededTimePerUser[threadIndex] += dur
							SuccessfulPipelineRuns[threadIndex] += 1
						}
						increaseBar(pipelinesBar, pipelinesBarMutex)
					}
